...
def grp_points(points):
    # print(points)
    temp_points = []
    for i, point in enumerate(zip(points[0], points[1], points[2], points[3])):
        # print(point)
        temp_points.append(list(point))

    temp_points = np.asarray(temp_points)
    temp_points = temp_points[temp_points[:, 1].argsort()]
    temp_points = split_list(temp_points, len(temp_points) // 64)
    ...
    for i, p in enumerate(temp_points):
        if i % 2 == 0:
            ...
            for v in p:
                temp.append(np.asarray([-100, -100, -100, -5]))
                # temp.append(v)
    x, y, z, d = [], [], [], []
    ...
    for p in temp:
        ...
        z.append(p[2])
        d.append(p[3])
    # print(np.asarray([x, y, z, d]))
    return np.asarray([x, y, z, d])
